...
class AnalyzeEtappb:
  ...
  def registryHist(self):
    ...
    self.__hist1D_5 = TH1F('h1_5', 'Probability of Proton useing both', 100, 0.000001, 1.0)
    self.__hist1D_5.GetXaxis().SetTitle('Probability of Proton')
    self.__hist1D_5.GetYaxis().SetTitle('Entries/')

    self.__hist1D_6 = TH1F('h1_6', 'Probability of Proton useing tof', 100, 0.000001, 1.0)
    self.__hist1D_6.GetXaxis().SetTitle('Probability of Proton')
    self.__hist1D_6.GetYaxis().SetTitle('Entries/')

    self.__hist1D_7 = TH1F('h1_7', 'Probability of Proton useing dedx', 100, 0.000001, 1.0)
    self.__hist1D_7.GetXaxis().SetTitle('Probability of Proton')
    self.__hist1D_7.GetYaxis().SetTitle('Entries/')

    self.__hist1D_8 = TH1F('h1_8', 'Probability of Kaon useing both', 100, 0.000001, 0.5)
    self.__hist1D_8.GetXaxis().SetTitle('Probability of Kaon')
    self.__hist1D_8.GetYaxis().SetTitle('Entries/')

    self.__hist1D_9 = TH1F('h1_9', 'Probability of Pion useing both', 100, 0.000001, 0.5)
    self.__hist1D_9.GetXaxis().SetTitle('Probability of Pion')
    self.__hist1D_9.GetYaxis().SetTitle('Entries/')

    self.__hist1D_10 = TH1F('h1_10', 'Run Map', 1000, 8000, 12000)
    self.__hist1D_10.GetXaxis().SetTitle('Run Map')
  ...
  def cut(self):
    ...
    if self.__t_diPho_mass_2[0] <= 0 or self.__t_diPro_mass_2[0] <= 0:
      return False
    ...
    #if mDiPho > 0.3:  
    #  return False
    #if mDiPro > 2.5 or mDiPro < 2.2:
    #  return False
    
    if mDiPho > 0.092 and mDiPho < 0.16:  
      return False
    if TMath.Abs(mDiPho - self.__eta_mass_mean) < self.__eta_mass_3sig:
      return False
    
    #if mDiPho > (self.__eta_mass_mean - self.__eta_mass_3sig):
    #  return False
    
    if mDiPho < 0.5 and\
        mDiPro > self.__f2.Eval(mDiPro):
      return False
    if mDiPho >= 0.5 and\
        mDiPro > self.__f1.Eval(mDiPho):
      return False
  ...
  def loop(self):
	...
	if self.__t_bothProbProt[0] > self.__t_bothProbPion[0] and\
	    ...
	    self.__t_bothProbProt[0] > 0.4 and\
	    self.__t_bothProbProt[1] > 0.4: 
          ...
          self.__hist1D_5.Fill(self.__t_bothProbProt[0])
          self.__hist1D_8.Fill(self.__t_bothProbKaon[0])
          self.__hist1D_9.Fill(self.__t_bothProbPion[0])
          self.__hist1D_10.Fill(self.__t_run[0])
          self.__hist1D_6.Fill(self.__t_tofProbProt[0] )
          self.__hist1D_7.Fill(self.__t_dedxProbProt[0])
	  #print 'Dedx Chi2: ', self.__t_pidDedxChi2[0]
