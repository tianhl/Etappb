...
class AnalyzeEtappb:
  def __init__(self, dst_name):
    ...
    self.registryInput(dst_name)  
    self.registryHist()  
    self.cutCondition()

  def registryInput(self, dst_name):  
    from ROOT import TTree, TFile, TH1F, TH2F, TPostScript, TCanvas
    self.__fi = TFile(dst_name)
    self.__tr = self.__fi.Get('analy')
  ...
  def cutCondition(self):  
    ...
    self.__eta_mass_3sig = 9 * eta_sig 
...
def test(dst_name, out_name):
  import os
  os.chdir(out_name)
  analyze = AnalyzeEtappb(dst_name) 
  analyze.loop()
  analyze.finalize()
...
if __name__ == '__main__':
  ...
  dst_name = os.path.join(os.path.normpath(os.path.abspath(sys.argv[1])), 'analy/test.root')
  out_name = os.path.join(os.path.normpath(os.path.abspath(sys.argv[1])), 'analy')
  test(dst_name, out_name)  
